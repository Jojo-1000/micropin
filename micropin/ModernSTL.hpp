...
namespace MicroPin
{
    ...
    {
        constexpr own_tuple(First first, Rest... rest) : own_tuple<Rest...>(rest...), head(first) { }
        First head;
    };
    ...
    template <typename... Ts>
    constexpr auto make_own_tuple(Ts&&... ts) -> decltype(own_tuple<decay_t<Ts>...>(forward<Ts>(ts)...))
    {
    ...
    constexpr auto own_tuple_cat(own_tuple<TupleTypes0...> t0, own_tuple<TupleTypes1...> t1, indices<Indices0...>,
        indices<Indices1...>) -> decltype(make_own_tuple(get<Indices0>(t0)..., get<Indices1>(t1)...))
    {
        return make_own_tuple(get<Indices0>(t0)..., get<Indices1>(t1)...);
    }
    template <typename... TupleTypes0, typename... TupleTypes1>
    constexpr auto own_tuple_cat(own_tuple<TupleTypes0...> t0, own_tuple<TupleTypes1...> t1) -> decltype(
        own_tuple_cat(t0, t1, typename build_indices<own_tuple_size<own_tuple<TupleTypes0...>>::value>::type {},
    ...
    struct own_array
    {
        ...
        // The non-const methods only work in c++14 or higher, because in c++11 all constexpr methods are const
    ...
    constexpr auto toArray(Tuple&& tuple) -> decltype(
        toArray<T>(forward<Tuple>(tuple), typename build_indices<own_tuple_size<decay_t<Tuple>>::value>::type {}))
    {
        return toArray<T>(
            forward<Tuple>(tuple), typename build_indices<own_tuple_size<decay_t<Tuple>>::value>::type {});
    }
} // namespace MicroPin
